...
use Dearvn\JobPlace\Abstracts\RESTController;
use Dearvn\JobPlace\Jobs\Job;
use WP_REST_Request;
...
class JobsController extends RESTController {
	...
	public function get_item( $request ) {
		...
		if ( ! $job ) {
			return new WP_Error( 'job_place_rest_job_not_found', __( 'Job not found. May be job has been deleted or you don\'t have access to that.', 'jobplace' ), array( 'status' => 404 ) );
		}
	...
	public function delete_items( $request ) {
		if ( ! isset( $request['ids'] ) ) {
			return new WP_Error( 'no_ids', __( 'No job ids found.', 'jobplace' ), array( 'status' => 400 ) );
		}
		...
		return new WP_Error( 'no_job_deleted', __( 'No job deleted. Job has already been deleted. Please try again.', 'jobplace' ), array( 'status' => 400 ) );
	}
	...
	public function sanitize_job_slug( $slug, $request ) {
		...
		if ( $total_found > 0 ) {
			return new WP_Error(
				'job_place_rest_slug_exists',
				__( 'Job slug already exists.', 'jobplace' ),
				array(
					'status' => 400,
				)
			);
