...
namespace Dearvn\JobPlace\User;
...
class Hooks {
	...
	public function __construct() {
		...
		add_action( 'user_new_form', array( $this, 'add_user_type_field' ) );
		add_action( 'edit_user_profile', array( $this, 'add_user_type_field' ) );

		// Save user type field.
		add_action( 'user_register', array( $this, 'save_user_type_field' ) );
		add_action( 'profile_update', array( $this, 'save_user_type_field' ) );
	}
	 ...
	 * @since 0.0.1
	 *
	 * @param \WP_User $user input value.
	 *
	 * @return void
	 */
		...
		<table class="form-table">
			<tr>
				...
				<td>
					...
					<input type="hidden" name="user_type_nonce" value="<?php echo esc_attr( wp_create_nonce( 'user_type_nonce' ) ); ?>" />
				</td>
			</tr>
		</table>
		<?php
